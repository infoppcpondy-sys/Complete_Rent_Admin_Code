...
const FollowUpGetTable = () => {
  ...
  const handleBulkPaidClose = async () => {
    ...
    try {
      ...
      // Show result message
      if (successCount > 0) {
        alert(`✅ Successfully marked ${successCount} follow-up(s) as Paid Closed!`);
      }
      if (errorCount > 0) {
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-3 space-x-2">
        ...
        <button className="btn btn-success ms-2" onClick={() => handleStatusFilter('Paid Closed')}>✓ Paid Closed</button>
        <button className="btn btn-danger ms-2" onClick={() => handleStatusFilter('Not Interested-Closed')}>✗ Not Interested-Closed</button>
        {/* <button 
          className="btn ms-2" 
